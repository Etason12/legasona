...
SERIALIZERS = {
    ...
    SparePart: lambda r: {'id': r.id, 'part_number': r.part_number, 'name': r.name, 'name_tigrinya': r.name_tigrinya, 'category': r.category, 'unit_price': str(r.unit_price) if r.unit_price else None, 'cost_price': str(r.cost_price) if r.cost_price else None, 'branch_id': r.branch_id, 'quantity': r.quantity, 'image': r.image},
