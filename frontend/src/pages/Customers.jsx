          ...
          <tr key={c.id} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/60 transition-colors group cursor-pointer" onClick={() => fetchDetails(c.id)}>
           ...
           <td className="px-6 py-4 text-right">
            <div className="flex items-center justify-end gap-2 transition-opacity" onClick={e => e.stopPropagation()}>
              ...
              className="p-2.5 bg-neutral-100 dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 rounded-lg text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
              title={t('edit')}
             ><Edit3 size={15}/></button>
             {isAdmin(user) && (
              <button onClick={(e) => handleDelete(c.id, e)} className="p-2.5 bg-rose-100 dark:bg-rose-900/30 hover:bg-rose-200 dark:hover:bg-rose-900/50 rounded-lg text-rose-600 transition-colors" title={t('delete')}><Trash2 size={15}/></button>
             )}
             <button onClick={() => fetchDetails(c.id)} className="p-2.5 bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-200 dark:hover:bg-blue-900/50 rounded-lg text-blue-600 dark:text-blue-400 transition-colors" title={t('viewHistory')}><ExternalLink size={15}/></button>
            </div>
           </td>
          </tr>
         ))}
