...
const Reports = ({ user }) => {
 ...
 const fetchReportData = async () => {
  try {
    ...
    const [statsRes, profitRes, paymentsRes] = await Promise.all([
 ...
 const branchData = [
  { name: 'Shire Branch', value: 2400000 },
  { name: 'Mekelle Branch', value: 2100000 }
 ];

 const vehicleTypeData = [
  { name: '2-Wheel', value: 45 },
  { name: '3-Wheel', value: 25 },
  { name: '4-Wheel', value: 30 }
 ];
        ...
        {payments.length === 0 ? (
         ...
         payments.map((p, i) => (
          <tr key={i} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
           ...
           <td className="py-3 px-2 text-slate-500">{p.bank_name || '—'}</td>
           <td className="py-3 px-2 text-slate-500">{p.account_holder || '—'}</td>
           <td className="py-3 px-2 text-slate-500 font-mono text-xs">{p.transaction_reference || '—'}</td>
          </tr>
         ))
        )}
       </tbody>
